...
import { IWalk, Dictionary, WalkName } from '../utils/types';
...
const transformForChart = (walks: IWalk[]): { [key: string]: IGraphDataPoint }  => {
    const data =  walks.reduce((accum: { [key: string]: IGraphDataPoint }, walk: IWalk): { [key: string]: IGraphDataPoint } => {
        const actTime: number = calculateActivityTime(walk.walkTimeStamps);
...
export const DawgResponsiveBar: React.FC<IProps> = ( { walkHistory } ) => {
  console.log("===> transformForChart", transformForChart(walkHistory))
